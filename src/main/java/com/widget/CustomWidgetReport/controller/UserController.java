...
public class UserController {
	...
	public Response<User> saveUser(@RequestBody UserDto userDto){
		...
		try {
			...
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "stack trace : " + Arrays.toString(e.getStackTrace()));
				return new Response<User>(500, false, HttpStatus.INTERNAL_SERVER_ERROR, "Error while creating user").setResponse(null);
			}
	...
	public Response<List<User>> getUsers() {
		...
		try {
			...
			users.removeIf(u->(u.getUserName().equalsIgnoreCase("superadmin") || u.getStatus().equalsIgnoreCase("D")));
		} catch (Exception e) {
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "Error while listing user : " + e);
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "stack trace : " + Arrays.toString(e.getStackTrace()));
			return new Response<List<User>>(500, false,HttpStatus.INTERNAL_SERVER_ERROR, "Error while getting users").setResponse(null);
		}
		return new Response<List<User>>(200, true, HttpStatus.OK, users.size() +" users has been retrived").setResponse(users);
	}
	...
	public Response<User> updateUser(@PathVariable Integer userId, @RequestBody UserDto userDto){
		...
		try {
			...
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "stack trace : " + Arrays.toString(e.getStackTrace()));
				return new Response<User>(500, false, HttpStatus.INTERNAL_SERVER_ERROR, "Error while updating user").setResponse(null);
			}
			return new Response<User>(200, true, HttpStatus.OK, "User has been updated").setResponse(user);
		
	}
	
	/*@PutMapping("/delete/{userId}/updatedBy/{updatedBy}")
	public Response<User> deleteUser(@PathVariable Integer userId, @PathVariable String updatedBy){
		DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"); 
		User user = null;
		try {
				user= userService.findById(userId);
			...
			log.error("stack trace : " + Arrays.toString(e.getStackTrace()));
				return new Response<User>(500, false, HttpStatus.INTERNAL_SERVER_ERROR, "Error while deleting user").setResponse(null);
			}
			return new Response<User>(200, true, HttpStatus.OK, "User has been deleted").setResponse(user);
	...
	public Response<String> deleteUserById(@PathVariable Integer userId) {
		...
		log.info(CustomWidgetLog.getCurrentClassAndMethodName() + "user got deleted--->"+user);
				...
				return new Response<String>(500, false,HttpStatus.NOT_FOUND, "No user found with the userId : " + userId).setResponse(null);
			}
			userService.deleteById(userId);
		} catch (Exception e) {
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "Error while deleting user by id : " + e);
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "stack error : " + Arrays.toString(e.getStackTrace()));
			return new Response<String>(500, false,HttpStatus.INTERNAL_SERVER_ERROR, "Error while deleting user").setResponse(null);
		}
...
//		String usercampaign;
	...
	public Response<User> findByUserName(@PathVariable String userName){
		...
		try {	
			...
			log.error(CustomWidgetLog.getCurrentClassAndMethodName() + "stack trace : " + Arrays.toString(e.getStackTrace()));
			return new Response<User>(500, false, HttpStatus.INTERNAL_SERVER_ERROR, "Error while getting user by username").setResponse(null);
		}
		
		if(user==null) {
			return new Response<User>(404, false, HttpStatus.NOT_FOUND, "No user found with User name : " + userName).setResponse(null);
		}
